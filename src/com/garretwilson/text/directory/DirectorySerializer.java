...
public class DirectorySerializer implements DirectoryConstants
{
	...
	final Map valueSerializerMap=new HashMap();	
		...
		@param valueSerializer The value serizlier to be registered with this value type.
		*/	
		...
		@return A value serializer for this value type, or <code>null</code> if
			there is no value serializer registered for this value type.
		*/ 
	...
	*/
	...
	/**Default constructor.
	This class automatically registers the predefined profile as a value
		serializer for standard value types.
	*/
	...
	*/
	...
	<p>The delimiter after the content line will be written.</p>
	<p>When attempting to find a <code>ValueSerializer</code> to process a given
		value, an attempt is made to locate a value serializer in this order:</p>
	<ol>
		...
		<li>If a profile name is known and the
			...
			<code>ValueSerializer</code>, it is asked to serialize the value object.</li>
		<li>If the value was not serialized created, if the value type is known,
			the <code>ValueSerializer</code> registered for the type, if any, is asked
			to serialize the value object.</li>
		<li>If the value was not serialized, the string value of the object value
			is serialized.</li> 
	</ol> 
...
//G***del Debug.trace("Serializing content line for type name: ", typeName);
		...
		else if(END_TYPE.equalsIgnoreCase(typeName))	//if this is END:xxx
		{
			...
			{
				//G***fix or del throw new ParseIOException("Profile \""+profile+"\" END without BEGIN.", reader);	//throw an error indicating that there was no beginning to the profile
			}
		}
...
//G***del Debug.trace("using profile type: ", typeName);
	...
	/**Serializes a directory content line's parameters.
	If multiple parameters exist with identical names, compared without regard to
		case, the values will be combined into multiple values for a single
		parameter.
	The parameter delimiters will be written, including the beginning parameters
		separator that appears before any parameters. 
